...
def test_read_one_authenticated(test_todo):
    response = client.get(f"/todos/todos/{test_todo.id}")
    assert response.status_code == status.HTTP_200_OK
    assert response.json() == {"completed": False, "description": "Need to learn FastAPI", "id": 1, "owner_id": 1, "priority": 5, "title": "Learn to code"}
...
def test_update_todo(test_todo):
    request_data ={
        "title": "Updated Todo",
        "description": "Need to learn FastAPI",
        "priority": 5,
        "completed": False,
    }

    response = client.put(f"/todos/todos/{test_todo.id}", json=request_data)
...
def test_delete_todo(test_todo):
    response = client.delete(f"/todos/todos/1")
    assert response.status_code == status.HTTP_204_NO_CONTENT
    db = TestingSessionLocal()
    model = db.query(Todos).filter(Todos.id == 1).first()
...
def test_delete_todo_not_found():
    response = client.delete(f"/todos/todos/999")
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {"detail": "Todo not found"}
